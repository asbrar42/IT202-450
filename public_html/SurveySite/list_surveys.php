...
if (isset($current_user)) {
    try{
        $stmt = $db->prepare("SELECT id, title FROM survey1 WHERE status!='public' AND user_id=$current_user");
    ...
    try{
        $stmt = $db->prepare("SELECT id, title FROM survey1 WHERE status='public'");
        $stmt->execute(array("%$query%"));
        foreach($stmt as $result ){
            echo "<ul>";
